...
let lhs_to_exp loc info l =
  ...
  mk
    (match l.ldesc with
     | LHVar v -> EVar v
     | LHArr (v, e) -> EArrIndex (mk (EVar v), e)
     | LHField (v, _, s) -> EField (mk (EVar v), s)
    )

let exp_to_lhs e =
  match e.edesc with
  | EVar v -> Some (mk_t_lhs (LHVar v) e.eloc e.einfo)
  | EArrIndex ({edesc = EVar v}, e) ->
     Some (mk_t_lhs (LHArr (v, e)) e.eloc e.einfo)
  | EField (({edesc = EVar v} as s), f) ->
