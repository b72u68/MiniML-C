...
type ctx = typ VMap.t * (string * typ) list VMap.t 
...
and typecheck_exp (ctx: ctx) (e: p_exp) : t_exp =
  ...
  | EVar v ->
     (try
        mk_t_exp (EVar v) e.eloc (lookup_var ctx v)
...
let add_var (vctx, tctx) (t, s) =
  (VMap.add s t vctx, tctx)
...
let rec typecheck_def (ctx: ctx) (d: p_def) : ctx * t_def =
  ...
  | DFun (s, t, b) ->
     (match t with
      | TFunction (ret, argts) ->
         let ctx' = List.fold_left add_var ctx argts in
         let ctx' = add_var ctx' (t, s) in
          ...
          *)
         (add_var ctx (t, s), mk_def (DFun (s, t, b')) d.dloc)
         ...
         (fun (ctx, defs) (s, t, e) ->
           ...
           | Some e ->
              ...
              if teq t e'.einfo then
                (add_var ctx (t, s), (s, t, Some e')::defs)
              else t_error t e'.einfo e'.eloc
           | None -> (add_var ctx (t, s), (s, t, None)::defs)
         )
         (ctx, [])
         defs
     in
...
and typecheck_stmt (rtype: typ) (ctx: ctx) (s: p_stmt) : ctx * t_stmt =
  ...
  match s.sdesc with
  | SDecl (v, t, None) -> (add_var ctx (t, v),
                           mk_stmt (SDecl (v, t, None)) s.sloc)
  | SDecl (v, t, Some e) ->
     let ctx' = add_var ctx (t, v) in
  ...
  | SWhile (e, body) ->
     ...
     if teq e'.einfo TBool then
       ...
       (ctx, mk_stmt (SWhile (e', body')) s.sloc)
     else
       t_error TBool e'.einfo e'.eloc
