...
and desugar_statement loc s =
  ...
  | IF (e, s1, s2) -> [ms (C.SIf (desugar_expression loc e,
                                  dsb s1,
                                  dsb s2))]
  | WHILE (e, s) -> [ms (C.SWhile (desugar_expression loc e,
                                   dsb s))]
  | DOWHILE (e, s) ->
     let s' = ds s in
     s' @ [ms (C.SWhile (desugar_expression loc e, ms (C.SBlock s')))]
  | FOR (e1, e2, e3, s) ->
     [ms (C.SExp (desugar_expression loc e1));
      ms (C.SWhile (desugar_expression loc e2,
                    ms (C.SBlock
                          ((ds s)
                           @ [ms (C.SExp (desugar_expression loc e3))]))))]
  | BREAK -> [ms C.SBreak]
...
and desugar_expression loc e =
  ...
  | VARIABLE s -> C.mk_exp (C.EVar s) loc
